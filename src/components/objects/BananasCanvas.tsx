...
// @ts-ignore
function OneBanana({ index, z, speed }) {
  const ref = useRef();
  ...
  const { nodes, materials } = useGLTF("glb/banana.glb");
...
function Bananas({
  ...
  return (
    ...
    >
      ...
      {Array.from(
        { length: count },
        (_, i) => <OneBanana key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */
      )}
